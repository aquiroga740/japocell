<?php
    ...
    if($telefono!="" && $telefono!=$datos['usuario_telefono']){
            $check_telefono=conexion();
            $check_telefono=$check_telefono->query("SELECT usuario_telefono FROM usuario WHERE usuario_telefono='$telefono'");
            if($check_telefono->rowCount()>0){
                echo '
                    <div class="notification is-danger is-light">
                        <strong>¡Ocurrio un error inesperado!</strong><br>
                        El telefono ingresado ya se encuentra registrado, por favor elija otro
                    </div>
                ';
                exit();
            }
            ...
            echo '
                <div class="notification is-danger is-light">
                    <strong>¡Ocurrio un error inesperado!</strong><br>
                    Ha ingresado un telefono no valido
                </div>
            ';
            exit();
        } 
    ...
    $actualizar_usuario=$actualizar_usuario->prepare("UPDATE usuario SET usuario_nombre=:nombre,usuario_apellido=:apellido,usuario_usuario=:usuario,usuario_clave=:clave,usuario_email=:email,usuario_telefono=:telefono WHERE usuario_id=:id");
