<?php
	...
	if(isset($busqueda) && $busqueda!=""){
		...
		$consulta_datos="SELECT * FROM clientes WHERE cliente_nombre LIKE '%$busqueda%' OR cliente_telefono LIKE '%$busqueda%' ORDER BY cliente_nombre ASC LIMIT $inicio,$registros";

		$consulta_total="SELECT COUNT(cliente_id) FROM clientes WHERE cliente_nombre LIKE '%$busqueda%' OR cliente_telefono LIKE '%$busqueda%'";
		...
		foreach($datos as $rows){
			$tabla.='
				<tr class="has-text-centered" >
                    ...
                    <td>'.substr($rows['cliente_telefono'],0,25).'</td>
                    <td>
                        <a href="index.php?vista=product_client&client_id='.$rows['cliente_id'].'" class="button is-link is-rounded is-small">Ver clientes</a>
                    </td>
                    <td>
